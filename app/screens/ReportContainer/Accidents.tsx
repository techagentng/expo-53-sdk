...
const Accidents = ({ navigation }: any) => {
  ...
  const mediaAccess = async () => {
    try {
      ...
      Alert.alert("Error accessing media library", (error as Error).message);
    } finally {
      setImageLoading(false);
    }
  };
    ...
    try {
      ...
      if (!result.canceled) {
        ...
        for (let video of selectedVideos) {
          ...
          const fileSizeInMB = (videoInfo.exists && videoInfo.size) ? videoInfo.size / (1024 * 1024) : 0; // Convert size to MB
  ...
  async function uploadMediaFile() {
    try {
      ...
      mediaFormData.append("report_id", reportTypeID as string);
  ...
  async function submitReport() {
    try {
      ...
      if ((error as any).response) {
        ...
        console.log("Axios error:", (error as any).message);
