...
const MakeReport = ({ navigation }: any) => {
  ...
  async function submitReport() {
    ...
    try {
      ...
      console.log('Sub Report Type:', reportType);
      console.log('Description:', textInput);
      console.log('State:', selectedState);
      console.log('Local Gov:', selectedLocalGov);
      console.log('Is Anonymous:', isEnabled);
      console.log('Address:', address);
      console.log('Location:', location);
      console.log('Video Media:', videoMedia);
      console.log('Albums (Images):', albums);
      ...
      if (albums && albums.length > 0) {
        console.log('Adding images to report...');
        albums.forEach((uri, index) => {
          const fileType = uri.substring(uri.lastIndexOf('.') + 1);
          // For React Native, append file URI as string - server will handle file reading
          formData.append('mediaFiles', uri);
          formData.append('mediaTypes', `image/${fileType}`);
          formData.append('mediaNames', `image_${index}.${fileType}`);
        });
      }

      if (videoMedia && videoMedia.length > 0) {
        console.log('Adding videos to report...');
        videoMedia.forEach((uri, index) => {
          const fileType = uri.substring(uri.lastIndexOf('.') + 1);
          // For React Native, append file URI as string - server will handle file reading
          formData.append('mediaFiles', uri);
          formData.append('mediaTypes', `video/${fileType}`);
          formData.append('mediaNames', `video_${index}.${fileType}`);
        });
      }
      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          if (error.response.status === 401) {
            ...
            setErrorMessage(
              error.response.data.message || "There was an issue with the server. Please try again later."
            );
          }
        } else if (error.request) {
          console.log("network error:", error.message);
  ...
  function submitPost() {
    ...
    if (!reportType || !textInput || !selectedState || loading) {
      return false;
    }
