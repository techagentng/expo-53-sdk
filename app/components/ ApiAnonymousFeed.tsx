...
// Fix missing import - CustomActivityIndicator doesn't exist, using ActivityIndicator instead
const CustomActivityIndicator = ActivityIndicator;

const screenWidth = Dimensions.get("window").width;

const ApiAnonymousFeed = ({ item }: { item: FeedItem }) => {
  ...
  async function bookmarkfunc(id: string | number | undefined) {
    if (!id) {
      Alert.alert("Error", "Cannot bookmark post without an ID");
      return;
    }
    
    // For anonymous feed, show sign up prompt
    Alert.alert(
      "Sign Up Required",
      "You need to sign up to bookmark posts",
      [
        {
          text: "OK",
          style: "default"
      ...
      >
        <View style={styles.profileContainer}>
          {item?.user_is_anonymous === true ? (
            ...
            <Image
              source={
                (isValidImage && item?.thumbnail_urls
                  ? { uri: item.thumbnail_urls }
                  : require("@/assets/images/citizenx.png")) as any
              }
