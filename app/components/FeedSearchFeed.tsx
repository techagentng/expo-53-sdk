...
// Define types for the feed item
interface FeedItem {
  id?: string;
...
const ApiFeed = ({ item }: { item: FeedItem }) => {
  ...
  const [downCount, setDownCount] = useState(item?.downvote_count || 0);
  ...
  async function bookmarkfunc(id: string) {
    if (!id) return;
    const access_token = await AsyncStorage.getItem("access_token");
    // bookmarkPost now only expects { postId } and determines bookmark status internally
    dispatch(bookmarkPost({ postId: id }));
  }
  ...
  async function upVoteClick(postId: string) {
    setupvote((prev) => !prev);
    setdownvote(false);
    setVoteLoading(true);
    if (upvote === false) {
  ...
  async function downVoteClick(postId: string) {
